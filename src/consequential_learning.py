...
def collect_data(pi, gt_dist, num_samples, fraction_protected, use_s):
    x, s = gt_dist.sample_features(num_samples, fraction_protected)
    decisions = pi.make_decisions(x, s, use_s)
...
def train(**training_args):
    ...
    pi = LogisticPolicy(training_args["fairness_rate"], training_args["cost_factor"], training_args["fairness_function"], training_args["feature_map"], training_args["dim_x"], training_args["dim_s"])
    ...
    for i in range(1, training_args["time_steps"] + 1):        
        ...
        data = collect_data(pi, gt_dist, training_args["num_decisions"], training_args["fraction_protected"], (training_args["dim_s"] is not None))
